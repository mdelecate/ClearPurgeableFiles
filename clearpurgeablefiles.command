...
then cp ~/Desktop/ClearPurgeableSpace/largefile4G{,"-$(perl -MTime::HiRes -e 'printf("%.0f\n",Time::HiRes::time()*1000)')"} && waiting=0
elif [ 2000 -lt $diskspace ]
then cp ~/Desktop/ClearPurgeableSpace/largefile500M{,"-$(perl -MTime::HiRes -e 'printf("%.0f\n",Time::HiRes::time()*1000)')"} && waiting=0
elif [ 800 -lt $diskspace ]
then cp ~/Desktop/ClearPurgeableSpace/largefile100M{,"-$(perl -MTime::HiRes -e 'printf("%.0f\n",Time::HiRes::time()*1000)')"} && waiting=0
elif [ 500 -lt $diskspace ]
then cp ~/Desktop/ClearPurgeableSpace/largefile100M{,"-$(perl -MTime::HiRes -e 'printf("%.0f\n",Time::HiRes::time()*1000)')"} && waiting=1
elif [ $waiting -eq 1 ]
